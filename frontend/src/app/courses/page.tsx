        ...
        if (res.ok) {
          ...
          if (apiCourses && apiCourses.length > 0) {
            const mapped: Course[] = apiCourses.map((c: any) => ({
              ...
              highlights: ["CBCT & Digital Diagnostics", "Hands-On Surgery", "Live Clinical Protocol"],
              level: "Mastery Level",
            }));
      ...
      <section className="relative pt-40 pb-20 px-6 md:px-12 bg-gradient-to-b from-[#0F172A] via-slate-900 to-slate-950 overflow-hidden border-b border-slate-800">
        ...
        <div className="max-w-7xl mx-auto text-center relative z-10 space-y-6">
          ...
          <p className="text-slate-400 text-base sm:text-lg max-w-2xl mx-auto font-body leading-relaxed">
            Hands-on surgical & aesthetic training modules designed for dental practitioners, led by{" "}
            <Link
              href="/#doctor"
              className="text-slate-100 underline underline-offset-4 decoration-[#60A5FA]/60 hover:text-[#60A5FA] font-semibold transition-colors cursor-pointer"
            >
              Dr. Pratim Talukdar
            </Link>{" "}
            (Specialist Implantologist & Prosthodontist).
          </p>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-14 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-800 pb-8">
            ...
            <div className="text-xs text-slate-400 font-medium">
              Showing <span className="text-white font-bold">{coursesList.length}</span> Modules
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {coursesList.map((course) => (
              <div
                key={course.id}
                className="group rounded-[2rem] bg-slate-900 border border-slate-800/90 overflow-hidden shadow-2xl hover:border-slate-700 transition-all duration-500 flex flex-col justify-between"
              >
                {/* Course Banner Image */}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-6">
                  <div>
                    <h3 className="text-xl font-light text-white mb-3 group-hover:text-[#60A5FA] transition-colors">
                      {course.title}
                    </h3>
                    ...
                    <div className="space-y-2 mb-6">
                      <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Key Module Highlights:</p>
                      <div className="flex flex-wrap gap-2">
                        {course.highlights.map((h, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-slate-300 text-xs font-medium"
                          >
                  ...
                  <div className="pt-4 border-t border-slate-800 flex items-center justify-between gap-4">
                    <button
                      ...
                      className="flex-1 py-3.5 px-4 rounded-xl bg-[#28395C] hover:bg-slate-800 text-white text-xs font-bold uppercase tracking-widest transition-all duration-300 border border-white/10 flex items-center justify-center gap-2 cursor-pointer group/btn"
                    >
                    ...
                    <button
                      onClick={() => handleEnquire(course)}
                      className="w-11 h-11 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 flex items-center justify-center transition-colors cursor-pointer shrink-0"
          ...
          <div className="mt-16 p-8 sm:p-10 rounded-[2rem] bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-400">
                Contact{" "}
                <Link href="/#doctor" className="text-slate-200 underline font-semibold hover:text-[#60A5FA]">
                  Dr. Pratim Talukdar
                </Link>{" "}
                directly for customized group hands-on modules.
              </p>
            </div>
            <a
